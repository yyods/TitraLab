...
|-----|------|-------------|------------------|
| BTN1 | GPIO34 | SELECT / เลือก | เริ่ม / ยืนยัน / หยุด (แล้วแต่บริบท) |
| BTN2 | GPIO35 | UP / เลื่อนขึ้น | เพิ่มค่า (+5 mL, +0.5 pH) |
| BTN3 | GPIO39 | DOWN / เลื่อนลง | ยกเลิก (Cancel) ในทุกโหมด |

**การใช้งาน BTN1 ในแต่ละบริบท (Context-Dependent BTN1):**
| บริบท | การกด BTN1 |
|--------|------------|
| หน้าเมนูหลัก | เลือกโหมดที่ highlight |
| ก่อนเริ่มทำงาน (Ready screen) | เริ่มทำงาน (Start) |
| สอบเทียบ pH | ยืนยันค่า (Confirm reading) |
| Mode 5 Purge (ขณะปั๊มทำงาน) | หยุดปั๊ม (Stop pump) |
...
2. ระบบแสดงข้อมูลทดสอบ:
   - ปริมาตรเป้าหมาย: 5.00 mL (Target volume)
   - อัตราการไหล: ค่าที่สอบเทียบไว้ (Flow rate)
   - เวลาที่คำนวณ: time = 5.0 / flow_rate (Calculated time)
3. กดปุ่ม 1 (BTN1) เพื่อเริ่มสูบ หรือปุ่ม 3 (BTN3) ยกเลิก
4. ระบบจะสูบปริมาตร 5 mL ตามเวลาที่คำนวณ
5. วัดปริมาตรจริงที่ได้ ตรวจสอบว่าตรงกับค่าเป้าหมาย
6. ถ้าผลต่างมาก (เช่น %RSD > 5%) ให้สอบเทียบใหม่ (Mode 3)

**หมายเหตุ**: BTN3 สามารถยกเลิกได้ทั้งก่อนและระหว่างการสูบ
...
**ขั้นตอน (Manual Start/Stop):**
1. จุ่มปลายท่อดูด (suction tube) ในน้ำกลั่น
2. วางปลายท่อจ่าย (discharge tube) ในภาชนะเก็บของเสีย
3. เลือก Mode 5 จากเมนู
4. กด BTN1 เพื่อเริ่มสูบ หรือ BTN3 ยกเลิก
5. ปั๊มทำงานต่อเนื่องจนกว่าจะกด BTN1 อีกครั้งเพื่อหยุด
6. ระบบแสดงเวลาที่ใช้ล้างท่อเมื่อเสร็จ
...
```
เตรียมการ (Setup):
    - แสดงหน้าจอ Ready พร้อมพารามิเตอร์
    - รอกด BTN1 เพื่อเริ่ม หรือ BTN3 ยกเลิก
    ↓
เริ่มต้น: อ่านค่า pH เริ่มต้น (Volume = 0 mL)
    - รอ stabilize_time วินาทีก่อนหยดแรก
    ↓
วนซ้ำ (Loop) จนครบทุก step (เช่น 50 step):
    1. สูบสารไทแทรนต์ 0.2 mL (pump 0.2 mL at 100% duty)
    2. หยุดปั๊ม (stop pump)
    3. รอ stabilize_time วินาทีให้ pH เสถียร (default=10s)
    ...
    8. ที่ 4.80 mL: ส่งเสียงเตือนใกล้จุดสมมูล (3 beeps @ 2000 Hz)
    ↓
เงื่อนไขหยุด (Stop Conditions):
    - ครบทุก step (เช่น 50 step = 10.0 mL) - ไม่หยุดก่อนถึงจุดสมมูล
    - นิสิตกด BTN3 เพื่อยกเลิก (Cancel)
    ↓
วิเคราะห์: หาจุดสมมูลจากข้อมูลทั้งหมดด้วย detect_equivalence_point()
```

**หมายเหตุ**: ระบบเก็บข้อมูลครบ 50 จุดเพื่อให้วิเคราะห์ได้แม่นยำด้วย EquivPoint tool
...
| STABILIZE_TIME | 10.0 s | เวลารอ pH เสถียร (ปรับได้ใน main.py) |
| SAMPLE_VOLUME | 5.0 mL | ปริมาตรสารตัวอย่าง |
| MAX_VOLUME | 2 x sample | ปริมาตรไทเทรชันสูงสุด |
| ALERT_VOLUME | 4.80 mL | เสียงเตือนใกล้จุดสมมูล |
...
| การกระทำ | วิธีทำ |
|---------|-------|
| หยุดไทเทรชัน (Stop/Cancel) | กด BTN3 (DOWN) |
| ดูค่าปัจจุบัน | ดูจอ TFT หรือ Thonny terminal |
| เสียงเตือนใกล้จุดสมมูล | Buzzer ส่งเสียงสูง 3 ครั้ง (ที่ 4.80 mL) |
| เสียงเตือนเสร็จ | Buzzer ส่งเสียง 2 ครั้ง (เสร็จสิ้น) |

**การแสดงผลบนจอ TFT ระหว่างไทเทรชัน:**
- หมายเลข step ปัจจุบัน / step ทั้งหมด (เช่น Step 25/50)
- ปริมาตรรวม (เช่น 5.00 mL / 10.00 mL)
- ค่า pH ปัจจุบัน
- อุณหภูมิปัจจุบัน
...
**หมายเหตุสำคัญ:**
- ระบบจะเก็บข้อมูลครบทุก step (เช่น 50 จุดสำหรับ 10 mL)
- **ไม่หยุดอัตโนมัติเมื่อพบจุดสมมูล** - ต้องการข้อมูลครบสำหรับวิเคราะห์ด้วย EquivPoint
- เสียงเตือนที่ 4.80 mL เพื่อแจ้งนิสิตว่าใกล้จุดสมมูล
...
- ปรับปรุง Mode 5: เปลี่ยนเป็น manual start/stop (BTN1 เริ่ม, BTN1 หยุด)
- ปรับปรุง Mode 6: รอกด BTN1 ก่อนเริ่ม, เสียงเตือนที่ 4.80 mL, แสดงความคืบหน้าบน TFT
- อัปเดต Button Controls: BTN1 ทำงานตามบริบท, BTN3 เป็น Cancel ในทุกโหมด
