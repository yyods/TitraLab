...
if __name__ == "__main__":
    ...
    try:
        print("\n--- ทดสอบเสียงต่างๆ (Testing various sounds) ---")

        print("1. Beep สั้น (Short beep)")
        buzzer.beep()
        sleep_ms(500)

        print("2. Beep สองครั้ง (Double beep)")
        buzzer.beep_beep()
        sleep_ms(500)

        print("3. เสียงสำเร็จ (Success sound)")
        buzzer.success_sound()
        sleep_ms(500)

        print("4. เสียงผิดพลาด (Error sound)")
        buzzer.error_sound()
        sleep_ms(500)

        print("5. เสียงสอบเทียบ (Calibration sound)")
        buzzer.calibration_sound()
        sleep_ms(500)

        print("6. เสียงเตือน (Warning sound)")
        buzzer.warning_sound()
        sleep_ms(500)

        print("7. เสียงไทเทรตเสร็จ (Titration complete)")
        buzzer.titration_complete_sound()
        sleep_ms(500)

        print("8. ทดสอบทำนอง (Melody test)")
        # Do-Re-Mi-Fa-Sol
