...
if __name__ == "__main__":
    print("=" * 50)
    print("ทดสอบคลาส LED และ LEDManager")
    print("(Testing LED and LEDManager Classes)")
    print("=" * 50)

    # --- ทดสอบคลาส LED เดี่ยว ---
    # --- Test single LED class ---
    print("\n--- ทดสอบ LED เดี่ยว (Testing single LED) ---")

    led = LED(LED_RED, name="TestRed")
    print(led)

    try:
        print("1. เปิด LED (Turn on)")
        led.on()
        print(f"   สถานะ: {led}")
        sleep_ms(500)

        print("2. ปิด LED (Turn off)")
        led.off()
        print(f"   สถานะ: {led}")
        sleep_ms(500)
        ...
        print("4. กระพริบ 3 ครั้ง (Blink 3 times)")
        led.blink(3, 200)

        print("5. Pulse สั้น (Short pulse)")
        led.pulse(100)
    ...
    # --- ทดสอบ LEDManager ---
    # --- Test LEDManager ---
    print("\n--- ทดสอบ LEDManager (Testing LEDManager) ---")
    ...
    try:
        print("1. เปิด LED ทั้งหมด (All on)")
        leds.all_on()
        sleep_ms(500)

        print("2. ปิด LED ทั้งหมด (All off)")
        leds.all_off()
        sleep_ms(500)

        print("3. เปิด LED เขียว (Green on)")
        leds.green.on()
        sleep_ms(500)
        leds.green.off()

        print("4. เปิด LED แดง (Red on)")
        leds.red.on()
        sleep_ms(500)
        leds.red.off()

        print("5. กระพริบทั้งหมด (Blink all)")
        leds.all_blink(2, 200)

        print("6. แสดงสถานะสำเร็จ (Indicate success)")
        leds.indicate_success()
        sleep_ms(300)

        print("7. แสดงสถานะข้อผิดพลาด (Indicate error)")
        leds.indicate_error()
        sleep_ms(300)

        print("8. อ่านสถานะ (Get status)")
