...
|------|--------|---------|
| `data_calibrate.txt` | slope, intercept, R-squared | Week 2 pH Calibration |
| `data_flowrate.txt` | flow rate (mL/s) | Week 2 Pump Calibration |
...
5. บันทึกข้อมูลลง ESP32 flash storage (ไฟล์ CSV)
...
└─────────────┘      └─────────────┘      └─────────────┘      └─────────────┘
                            │                    │                    │
                            ▼                    ▼                    ▼
                     mV = ADC/4095 x 3300   pH = (mV - b) / m
```

### การตรวจจับจุดสมมูล (Equivalence Point Detection)

ระบบใช้วิธี derivative เพื่อหาจุดสมมูล:
...
ไฟล์ CSV ที่บันทึกลง ESP32 มีคอลัมน์:
```
Cycle,Time(s),Volume(mL),pH,Temperature(C)
1,2.50,0.100,2.50,25.1
2,5.00,0.200,2.55,25.1
...
```
...
*Version 3.1.0 - Modular OOP Titration System (ESP32 Flash Storage)*
