...
class AsyncPump:
    ...
    SUCCESS_THRESHOLD = 0.95  # เกณฑ์ความสำเร็จ 95% (95% success threshold)
    ...
    def create_task(self, coro):
        """
        สร้าง task และเก็บ reference สำหรับการยกเลิก
        Create task and store reference for cancellation

        Args:
            coro: coroutine ที่จะรัน (coroutine to run)
    ...
    async def pump_volume_async(self, volume_ml, callback=None):
        ...
        Raises:
            ValueError: ถ้า volume_ml ไม่เป็นค่าบวก (if volume_ml is not positive)
        ...
        """
        # ตรวจสอบค่า input (Validate input)
        if volume_ml <= 0:
            raise ValueError(f"volume_ml ต้องเป็นค่าบวก (volume_ml must be positive): {volume_ml}")
        ...
        if callback:
            success = not self._should_stop and actual_volume >= volume_ml * self.SUCCESS_THRESHOLD
            callback(actual_volume, success)
    ...
    async def purge_async(self, duration_s=5.0):
        ...
        Raises:
            ValueError: ถ้า duration_s ไม่เป็นค่าบวก (if duration_s is not positive)
        """
        # ตรวจสอบค่า input (Validate input)
        if duration_s <= 0:
            raise ValueError(f"duration_s ต้องเป็นค่าบวก (duration_s must be positive): {duration_s}")
