...
#
# ข้อจำกัดของ Priority (Priority Limitation):
# หมายเหตุสำคัญ: Priority มีผลเฉพาะลำดับการเริ่มต้น tasks เท่านั้น
# ไม่ส่งผลต่อการจัดตารางขณะรันไทม์ เนื่องจากข้อจำกัดของ cooperative multitasking
# IMPORTANT: Priority only affects the START ORDER of tasks, NOT runtime scheduling.
# This is a limitation of cooperative multitasking - once tasks are started,
# they all share CPU time based on when they yield (await), not their priority.
# ==============================================================================
...
class Task:
    ...
    Attributes:
        ...
        priority: ความสำคัญ (0=highest, 9=lowest)
                  หมายเหตุ: priority มีผลเฉพาะลำดับการเริ่มต้นเท่านั้น
                  Note: priority only affects start order, not runtime scheduling
        interval: ช่วงเวลาทำซ้ำ (repeat interval in seconds, None=once)
    """

    # ระดับความสำคัญ (Priority levels)
    # หมายเหตุ: Priority มีผลเฉพาะลำดับการเริ่มต้น tasks ไม่ใช่การจัดตารางขณะรันไทม์
    # Note: Priority only affects task START ORDER, not runtime scheduling
    ...
    def __init__(self, name, coro, priority=PRIORITY_MEDIUM, interval=None):
        ...
        Args:
            ...
            priority: ความสำคัญ (priority level)
                      หมายเหตุ: มีผลเฉพาะลำดับการเริ่มต้นเท่านั้น
                      Note: only affects start order, not runtime scheduling
...
class TaskScheduler:
    ...
    def add_task_from_factory(self, name, factory_result):
        """
        เพิ่ม task จาก factory function (เช่น create_sensor_reading_task)
        Add task from factory function result (e.g., create_sensor_reading_task)

        Factory functions คืนค่า tuple (coro, priority, interval)
        Factory functions return tuple (coro, priority, interval)

        Args:
            name: ชื่อ task (task name)
            factory_result: tuple (coro, priority, interval) จาก factory function

        Returns:
            Task: task object ที่สร้าง

        Example:
            factory_result = create_sensor_reading_task(sensor, callback, 1.0)
            scheduler.add_task_from_factory("sensor", factory_result)
        """
        coro, priority, interval = factory_result
        return self.add_task(name, coro, priority, interval)
        ...
        while not self._should_stop and task.is_enabled:
            try:
                ...
                if asyncio.iscoroutinefunction(task.coro):
                    ...
                    # เรียกฟังก์ชันปกติและ yield เพื่อให้ tasks อื่นทำงานได้
                    # Call regular function and yield to allow other tasks to run
                    task.coro()
...
# ==============================================================================
# การใช้งาน Factory Functions (Using Factory Functions):
# Factory functions คืนค่า tuple (coro, priority, interval) ซึ่งไม่สามารถ
# ส่งตรงไปยัง add_task() ได้ ให้ใช้ add_task_from_factory() แทน
#
# Factory functions return tuple (coro, priority, interval) which cannot
# be passed directly to add_task(). Use add_task_from_factory() instead.
#
# ตัวอย่าง (Example):
#   factory_result = create_sensor_reading_task(sensor, callback, 1.0)
#   scheduler.add_task_from_factory("sensor", factory_result)
...
def create_sensor_reading_task(sensor, callback, interval=1.0):
    ...
    Returns:
        tuple: (coro, priority, interval) - ใช้กับ add_task_from_factory()
               Use with add_task_from_factory()

    Example:
        factory_result = create_sensor_reading_task(ph_sensor, handle_ph, 1.0)
        scheduler.add_task_from_factory("ph_reading", factory_result)
    """
...
def create_display_update_task(display, data_provider, interval=0.5):
    ...
    Example:
        factory_result = create_display_update_task(tft, get_data, 0.5)
        scheduler.add_task_from_factory("display", factory_result)
    """
...
if __name__ == '__main__':
    ...
    print("""
    import uasyncio as asyncio
    from async_support.scheduler import (
        TaskScheduler, Task,
        create_sensor_reading_task,
        create_display_update_task
    )

    # สร้าง scheduler (Create scheduler)
    scheduler = TaskScheduler()

    # วิธีที่ 1: เพิ่ม task โดยตรง (Method 1: Add task directly)
    # หมายเหตุ: priority มีผลเฉพาะลำดับการเริ่มต้นเท่านั้น
    # Note: priority only affects start order, not runtime scheduling
    scheduler.add_task(
        "ph_sensor",
    ...
    # วิธีที่ 2: ใช้ factory functions (Method 2: Use factory functions)
    # Factory functions คืนค่า tuple ให้ใช้ add_task_from_factory()
    # Factory functions return tuples - use add_task_from_factory()
    sensor_task = create_sensor_reading_task(temp_sensor, handle_temp, 2.0)
    scheduler.add_task_from_factory("temperature", sensor_task)

    display_task = create_display_update_task(tft, get_display_data, 0.5)
    scheduler.add_task_from_factory("tft_update", display_task)
