"""
เฉลยแบบฝึกหัดที่ 2: คลาส Potentiometer (Exercise 2 Solution: Potentiometer Class)
=================================================================================
...
class Potentiometer:
    ...
    def __init__(self, pin_number: int):
        ...
        """
        # สร้าง ADC object จากขา GPIO (Create ADC object from GPIO pin)
        self.adc = ADC(Pin(pin_number))

        # ตั้งค่า attenuation สำหรับช่วงวัด 0-3.3V (Set attenuation for 0-3.3V range)
        # ATTN_0DB: 0-1.0V, ATTN_2_5DB: 0-1.34V, ATTN_6DB: 0-2.0V, ATTN_11DB: 0-3.3V
        self.adc.atten(ADC.ATTN_11DB)

        # กำหนดค่าดิบเริ่มต้น (Initialize raw value)
        self._raw_value = 0
    ...
    def read_average(self, samples: int = 10, delay_ms: int = 10) -> int:
        ...
        """
        # ตัวแปรสะสมค่า (Accumulator variable)
        total = 0

        # อ่านค่าตามจำนวน samples (Read specified number of samples)
        for i in range(samples):
            total += self.adc.read()
            time.sleep_ms(delay_ms)

        # คำนวณค่าเฉลี่ยและเก็บไว้ (Calculate average and store)
        self._raw_value = total // samples
        return self._raw_value
    ...
    def raw(self) -> int:
        ...
        Returns:
            int: ค่าดิบ 0-4095
        """
        # อ่านค่าใหม่และคืนค่า (Read new value and return)
    ...
    def percent(self) -> float:
        ...
        # สูตร: (ค่าที่อ่านได้ / ค่าสูงสุด) * 100
        return (raw / self.ADC_MAX) * 100.0
    ...
    def voltage(self) -> float:
        ...
        # คำนวณแรงดัน (Calculate voltage)
        # สูตร: (ค่าที่อ่านได้ / ค่าสูงสุด) * แรงดันอ้างอิง
        return (raw / self.ADC_MAX) * self.REF_VOLTAGE
    ...
    def map_to_range(self, min_val: float, max_val: float) -> float:
        ...
        # แปลงค่าด้วยสูตร linear interpolation
        # mapped = min + (raw / max_raw) * (max - min)
        return min_val + (raw / self.ADC_MAX) * (max_val - min_val)
